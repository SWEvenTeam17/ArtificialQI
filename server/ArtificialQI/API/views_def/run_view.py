...
class RunPromptView(APIView):
    def get(self, request):
        ...
        if not prompt_id:
            return Response(
                {"error": "Missing prompt_id."}, status=status.HTTP_400_BAD_REQUEST
            )
        try:
            prompt_id = int(prompt_id)
        except ValueError:
            return Response(
                {"error": "Invalid prompt_id."}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def delete(self, request):
        ...
        if not run_id:
            return Response(
                {"error": "Missing run_id."}, status=status.HTTP_400_BAD_REQUEST
            )
        try:
            run_id = int(run_id)
        except ValueError:
            return Response(
                {"error": "Invalid run_id."}, status=status.HTTP_400_BAD_REQUEST
            )
