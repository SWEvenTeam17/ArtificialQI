...
def test_blocktestview_get_success(monkeypatch, factory):
    ...
    monkeypatch.setattr(
        "API.views_def.block_view.LLMService.read", lambda x: FakeLLM(x)
    )
    monkeypatch.setattr(
        "API.views_def.block_view.BlockRepository.get_common_blocks_for_llms",
        lambda a, b: [FakeBlock(1, "block1")],
    )
    monkeypatch.setattr(
        "API.views_def.block_view.RunRepository.get_common_runs",
        lambda a, b, c: [FakeRun(FakeLLM(1), FakePrompt(), FakeEval(0.8, 0.9))],
    )
    ...
    monkeypatch.setattr(
        "API.views_def.block_view.Block",
        type("Block", (), {"objects": FakeBlockManager}),
    )
...
def test_blocktestview_get_no_common_blocks(monkeypatch, factory):
    ...
    monkeypatch.setattr(
        "API.views_def.block_view.RunRepository.get_common_runs", lambda a, b, c: []
    )
...
def test_blocktestview_get_multiple_runs(monkeypatch, factory):
    ...
    monkeypatch.setattr(
        "API.views_def.block_view.BlockRepository.get_common_blocks_for_llms",
        lambda a, b: [FakeBlock(1, "block1"), FakeBlock(2, "block2")],
    )
    monkeypatch.setattr(
        "API.views_def.block_view.RunRepository.get_common_runs",
        lambda a, b, c: [
