...
class TestRunRepository(TestAbstractRepository):
    ...
    def setup_data(self, db):
        ...
        _evaluation2 = Evaluation.objects.create(
            semantic_evaluation=95, external_evaluation=95
        )
        return {
            "llm": _llm,
            "llm2": _llm2,
    ...
    def test_commons_run(self, repository, valid_data, setup_data):
        ...
        assert (
            len(repository.get_common_runs(setup_data["llm3"], setup_data["llm3"], []))
            == 0
        )
        assert (
            len(repository.get_common_runs(setup_data["llm"], setup_data["llm2"], []))
            == 2
        )
